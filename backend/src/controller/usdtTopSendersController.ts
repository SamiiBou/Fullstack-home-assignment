...
export async function getUsdtTopSenders(req: Request, res: Response) {
  ...
  try {
    const data = await fetcher.getTopSendersForPeriod(provider, startTime, endTime);
    ...
    console.error("getUsdtTopSenders error:", err);
